...
class SOM_RK(SOM):
    ...
    def runge_kutta_update(self, weight, target, learning_rate, influence):
        # RK4 implementation for weight updates
        h = learning_rate
        k1 = h * influence * (target - weight)
        k2 = h * influence * (target - (weight + k1/2))
        k3 = h * influence * (target - (weight + k2/2))
        k4 = h * influence * (target - (weight + k3))
        return weight + (k1 + 2*k2 + 2*k3 + k4) / 6
    ...
    def train(self, data, num_epochs=100, init_learning_rate=0.01, resetWeights=False):
        ...
        for i in range(1, num_epochs + 1):
            ...
            if i % vis_interval == 0:
                if fig is not None:
                    self.show_plot(fig, int(i/vis_interval), i)
                print(f"SOM_RK training epoch {i}")
                ...
                for x in range(self.network_dimensions[0]):
                    for y in range(self.network_dimensions[1]):
                        ...
                        if w_dist <= radius ** 2:
                            ...
                            new_w = self.runge_kutta_update(weight, row_t, learning_rate, influence)
                            self.net[x, y, :] = new_w.reshape(1, self.num_features)
